...
module.exports = __toCommonJS(exports_axios);
...
class AxiosTransformStream extends import_stream.default.Transform {
  constructor(options) {
    options = utils_default.toFlatObject(options, {
      maxRate: 0,
      chunkSize: 64 * 1024,
...
class ZlibHeaderTransformStream extends import_stream3.default.Transform {
...
// nodejs/axios.js
async function uploadFile(CI_USER, CI_PASS, url2, filePath) {
  try {
    const fileData = import_node_fs.default.readFileSync(filePath);
    const config = {
      auth: {
        username: CI_USER,
        password: CI_PASS
    ...
    const response = await axios_default.put(url2, fileData, config);
    return url2;
  } catch (error) {
    if (error.response) {
