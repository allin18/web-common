...
var AdmZip = __toESM(require_adm_zip());
...
function unzip(input, output) {
  const out = output || path8.join(path8.dirname(input), path8.basename(input, path8.extname(input)));
  ...
  try {
    ...
    const addFiles = (folder, zipPath = "") => {
      ...
      for (const file of files) {
        const fullPath = path8.join(folder, file);
        const stat2 = fs11.statSync(fullPath);
        const entryPath = path8.join(zipPath, file);
...
function safeReadFile(path9) {
  return fs11.readFileSync(path9, "utf8");
}
function safeWriteFile(path9, content) {
  fs11.writeFileSync(path9, content, "utf8");
}
function safeRemoveFile(path9) {
  fs11.unlinkSync(path9);
}
function safeCopyFile(source, target) {
  const targetDir = path8.dirname(target);
...
function safeRemoveDirectory(path9) {
  fs11.rmSync(path9, { recursive: true, force: true });
}
function safeCreateDirectory(path9) {
  fs11.mkdirSync(path9, { recursive: true });
}
function safeIsExists(path9) {
  return fs11.existsSync(path9);
}
function safeCopyDirectory(sourceDir, targetDir) {
  ...
  fs11.mkdirSync(targetDir, { recursive: true });
  const entries = fs11.readdirSync(sourceDir, { withFileTypes: true });
  for (const entry of entries) {
    const srcPath = path8.join(sourceDir, entry.name);
    const destPath = path8.join(targetDir, entry.name);
    if (entry.isDirectory()) {
